...
class ResourceMixin(object):
    ...
    @classmethod
    def bulk_delete_uuid(cls, ids):
        """
        Delete 1 or more model instances after ensuring that all references in other tables are cleared.
        ...
        # First, delete or update the referenced rows in 'gpt' table or handle them as needed
        # gpt_delete_count = Gpt.query.filter(Gpt.conversation_uuid.in_(ids)).delete(synchronize_session=False)
        
        gpt_delete_count = Gpt.query.filter(Gpt.conversation_uuid.in_(ids)).update(
            {"user_deleted": True},
            synchronize_session=False
        )

        # Now, delete the conversations after the references have been cleared
        # delete_count = cls.query.filter(cls.conversation_uuid.in_(ids)).delete(synchronize_session=False)
        delete_count = cls.query.filter(cls.conversation_uuid.in_(ids)).update(
            {"user_deleted": True},
            synchronize_session=False
        )
        
        # Commit changes if all deletions were successful
        db.session.commit()
